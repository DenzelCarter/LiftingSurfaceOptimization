...
MODEL_SAVE_PATH = 'Trained_Models/prop_efficiency_model.joblib'
PLOTS_FOLDER = 'Plots'
...
# ===================================================================
print(f"Holding out the following props for the test set: {TEST_PROP_FILENAMES}")
...
plt.xlabel("Actual Propeller Efficiency")
plt.ylabel("Predicted Propeller Efficiency")
plt.title(f"Predicted vs. Actual Performance (R² = {r2:.3f})")
plt.legend()
plt.grid(True)
plt.tight_layout()

perf_save_path = os.path.join(PLOTS_FOLDER, 'prop_model_performance_predicted_vs_actual.pdf')
...
plt.title("Feature Importance for Propeller Efficiency Model")
plt.xlabel("Importance Score")
plt.ylabel("Feature")
plt.tight_layout()

imp_save_path = os.path.join(PLOTS_FOLDER, 'prop_model_feature_importance.pdf')
