...
X_FEATURES = [
    ...
    'Motor Electrical Speed (RPM)', 'flexMod (GPA)',
    'avg_thickness_m', 'reynolds_number','prop_eff_bemt'
]
...
# ===================================================================
print("--- Training Propeller Efficiency Model ---")
...
print(f"\nPerforming Leave-One-Group-Out Cross-Validation on {len(np.unique(groups))} props...")
scores = cross_val_score(model, X, y, cv=logo, groups=groups, scoring='r2')

print("\n--- Cross-Validation Results ---")
print(f"R² scores for each holdout prop: {np.round(scores, 3)}")
